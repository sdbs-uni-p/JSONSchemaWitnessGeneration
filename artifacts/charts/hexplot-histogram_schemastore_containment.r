...
source('config.r')
df <- read.csv('data/schemastore_containment/clean_data.csv')
df_timeout <- read.csv('data/schemastore_containment/timeout.csv')
...
hexplot_fix <- function(col.low="#132B43", col.high="#56B1F7", col.med="red", col.inf="red", y_title="") {
  ...
  plot <- ggplot(data=df, aes(x = inSize, y=totalTime)) + 
    ...
    geom_text(aes(x=size_median, label="Median", y=10^5.8), nudge_x=-0.4, nudge_y=-0.2, colour=col.med, 
    ...
    geom_text(aes(x=10^5.5, label="Median", y=time_median), nudge_x=0.1, nudge_y=0.6, colour=col.med, 
...
ggsave(paste("output/hexplot_and_histograms_schemastore_containment_fix",file_ending,sep=""), plot=plot, height=5, width=8.5, device=cairo_pdf)
