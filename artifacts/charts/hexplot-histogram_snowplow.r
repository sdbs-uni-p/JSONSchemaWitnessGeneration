...
source('config.r')
df <- read.csv('data/snowplow/clean_data.csv')

hexplot_lim <- function(col.low="#132B43", col.high="#56B1F7", col.med="red", col.inf="red", y_title="") {
  ...
  plot <- ggplot(data=df, aes(x = inSize, y=totalTime)) + 
    ...
    geom_text(aes(x=10^5.8, label="Median", y=time_median), nudge_x=-0.5, nudge_y=0.6, colour=col.med, 
...
plot <- hexplot_lim(col.low="#56B1F7", col.high="#132B43", y_title="Time [ms]")
plot
ggsave(paste("/home/repro/results/charts/hexplot_and_histograms_snowplow_fix",file_ending,sep=""), plot=plot, height=5, width=8.5, device=cairo_pdf)
...
hexplot <- function(col.low="#132B43", col.high="#56B1F7", col.med="red", col.inf="red", y_title="") {
  ...
  plot <- ggplot(data=df, aes(x = inSize, y=totalTime)) + 
    ...
    geom_text(aes(x=size_median, label="Median", y=10^6), nudge_x=-0.2, nudge_y=-0.2, colour=col.med, 
              angle=90, family=text_family, check_overlap = TRUE, size=7) +
    geom_hline(yintercept=time_median, size=.5, color=col.med, linetype = "dashed") + 
    geom_text(aes(x=10^4.8, label="Median", y=time_median), nudge_x=-0.5, nudge_y=0.6, colour=col.med, 
              family=text_family, check_overlap = TRUE, size=7) +
    labs(x = "File Size [Bytes]", y = y_title) +
    scale_fill_gradient(name="Count", low=col.low, high=col.high) +
    scale_y_continuous(trans = "pseudo_log", breaks = 10^(0:7),
...
plot
ggsave(paste("/home/repro/results/charts/hexplot_and_histograms_snowplow",file_ending,sep=""), plot=plot, height=5, width=8.5, device=cairo_pdf)
