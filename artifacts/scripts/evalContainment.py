...
def extractAndCheck(pathToFolder):
    ...
    subFoldersList = [
        f for f in os.listdir(pathToFolder) if path.isdir(join(pathToFolder, f))
    ]
    for subFolder in subFoldersList:
        subFolderPath = join(pathToFolder, subFolder)
        subSubFolders = [
            f for f in os.listdir(subFolderPath) if path.isdir(join(subFolderPath, f))
        ]
...
def evalSubschema(df):
    ...
    notEqualDF = pd.DataFrame(
        notEqual_list, columns=["s1SUBs2", "IBM_s1SUBs2", "count"]
    )

    sat_logical_errors = notEqualDF[
        (notEqualDF["s1SUBs2"] == 0) & (notEqualDF["IBM_s1SUBs2"] == 1)
    ]
    unsat_logical_errors = (
        df[(df["s1SUBs2"] == 1)]["IBM_s1SUBs2"].value_counts(dropna=False).to_frame()
    )

    failure = (
        len(notEqual)
        - sat_logical_errors.iloc[0]["count"]
        - unsat_logical_errors.loc[0, "IBM_s1SUBs2"]
    )
    ...
    print(
        "\t\tCorrection: 43 schemas removed because they cause problems (cannot catch the exception)\n",
        "\t\t\t => Increased Failure by 43",
    )
    sat_err_perc = round(100 * sat_logical_errors.iloc[0]["count"] / 1331, 2)
    print(
        f"\tLogical errors sat: {sat_logical_errors.iloc[0]['count']} ({sat_err_perc})%"
    )
    unsat_err_perc = round(100 * unsat_logical_errors.loc[0, "IBM_s1SUBs2"] / 1331, 2)
    print(
        f"\tLogical errors unsat: {unsat_logical_errors.loc[0, 'IBM_s1SUBs2']} ({unsat_err_perc}%)"
    )
