...
//            processed=true;
    ...
    public Boolean call() throws IOException {
        ...
        if (!err || errors==null) {
            System.out.println("********* Trying the generation without the optimization oneOf as anyOf ******\n");
            OneOf.asAnyOf = false;

            // Resetting the maps after the generation failure using the optimization
            for (String k : this.resultMap.keySet())
                if(!(k.equals(Utils.objectId) || k.equals(Utils.inSize) || k.equals(Utils.dateAndTime) ||
                        k.equals(Utils.timeout) ||k.equals(Utils.git) || k.equals(Utils.machine) ||
                        k.equals(Utils.parsing) || k.equals(Utils.extractSchema)))
                    this.resultMap.put(k,"");

            for (String k : this.sizeMap.keySet())
                if(!(k.equals(Utils.objectId) || k.equals(Utils.inSize)))
                    this.sizeMap.put(k,"");
